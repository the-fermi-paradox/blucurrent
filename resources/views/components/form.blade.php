    ...
    <div class="row justify-content-center">
        <div class="col-md-6 bg-white" style="border-radius:.5rem">
            <form novalidate method="post" class="needs-validation">
                    ...
                    <div class="col">
                        <div class="dropdown">
                            <button
                                ...
                                aria-expanded="false"
                                data-id="{{ $empire !== null ? $empire->id : $releases->first()?->id }}"
                            >
                                {{ $empire !== null ? $empire->name : $releases->first()?->name }}
                            </button>
                ...
                <input type="text" id="release_id" name="release_id" class="d-none" value="{{ $empire !== null ? $empire->id : $releases->first()?->id }}" required/>
                <button data-mdb-ripple-init type="submit" class="btn btn-primary btn-block mb-4">Submit</button>
                <a href="{{ route('empire.list') }}" data-mdb-ripple-init type="button" class="btn btn-secondary btn-block mb-4">Cancel</a>
            </form>
        </div>
    </div>
</div>
